    ...
    public void saveFicha(Ficha ficha, HistorialPago hp) {
        ...
        if(ficha.getCliente() != null){
            if(!GV.getStr(ficha.getCliente().getCod()).isEmpty()){
                PreparedStatement consulta = LcBd.obtener().prepareStatement("SELECT * FROM cliente WHERE cli_rut='" + ficha.getCliente().getCod() + "'");
                ResultSet datos = consulta.executeQuery();
                while (datos.next()) {
                    LcBd.cerrar();
        ...
        if(ficha.getDoctor() != null){
            if(!GV.getStr(ficha.getDoctor().getCod()).isEmpty()){
                PreparedStatement consulta = LcBd.obtener().prepareStatement("SELECT * FROM doctor WHERE doc_rut='" + ficha.getDoctor().getCod() + "'");
                ResultSet datos = consulta.executeQuery();
                while (datos.next()) {
                    LcBd.cerrar();
